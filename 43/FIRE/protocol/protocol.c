  ...
  while(1)
  {
    ...
    switch (cmd_type)
    {
      ...
      case SET_P_I_D_CMD:
      {
		...
		SendPacket_PID(Channal01Address1Byte, p_temp, i_temp, d_temp ,USART1);	
        /* 在这里填写自己的处理函数 */
      ...
      case SET_TARGET_CMD://目标值
      {
        uint32_t actual_temp = COMPOUND_32BIT(&frame_data[13]);    // 得到数据
				
		Target_A = actual_temp;
		SendPacket_Target(Channal01Address1Byte,Target_A,USART1);
