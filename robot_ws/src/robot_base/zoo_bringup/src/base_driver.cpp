...
void BaseDriver::get_sensor()     //ID11
{
    if (need_get_sensor)
    {
        ROS_INFO_STREAM("get_sensor");
        Sensor_state* sen = &Data_holder::get()->sensor;
        need_get_sensor = !(frame->interact(ID_GET_SENSOR));
        ROS_INFO("Robot sensor:\r\n distance: 1:%d 2:%d 3:%d 4:%d 5:%d 6:%d \r\n infrared  1:%d 2:%d 3:%d 4:%d 5:%d 6:%d \r\n touch     1:%d 2:%d 3:%d 4:%d 5:%d 6:%d ", (sen->sound11+sen->sound12*256),(sen->sound21+sen->sound22*256),(sen->sound31+sen->sound32*256),(sen->sound41+sen->sound42*256),(sen->sound51+sen->sound52*256),(sen->sound61+sen->sound62*256),sen->infrared1,sen->infrared2,sen->infrared3,sen->infrared4,sen->infrared5,sen->infrared6,sen->touch1,sen->touch2,sen->touch3,sen->touch4,sen->touch5,sen->touch6);
